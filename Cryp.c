...
/**********************************************
* Simple crypto cli that will encrypt/decrypt * 
...
void usage() {
   printf("Usage: Cryp --enc <encryption> -f <file> \n");
}
...
int main(int argc , char **argv) {
   ...
   while ((option_arg= getopt_long(argc, argv, "e:x:f:d:vuh", long_options, NULL)) != -1) {
      ...
      case 'd': // directory
         ...
         // check & remove "/" from the end of the directory name
         if (arguments.directory[strlen(arguments.directory) -1 ] == '/' ) {
            arguments.directory[strlen(arguments.directory)-1] = '\0';
         }
         else {
            arguments.directory[127] = '\0';
         }
         break;
      case 'v': //version
         printf("\n%s", program_version);
   ...
   // handle directory mode 
      ...
      // printing sub directories files
      ...
      if ( enc_flag ) {
         ...
         printf(GRN "\n[+]" RESET " Please enter a 16 charecters long key to dec the files : ");
         scanf("%s19" , key);
